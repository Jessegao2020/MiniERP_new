...
public partial class CustomerPickerWindow : Window
{
    ...
    private void ApplyFilters()
    {
        ...
        foreach (var customer in filtered)
            _visibleCustomers.Add(customer);
        ...
        if (_currentCustomerId is not null && CustomerGrid.SelectedItem is null)
        {
            ...
            if (current is not null)
                CustomerGrid.SelectedItem = current;
        }

        StatusText.Text = _filters.Count == 0
            ? $"{_visibleCustomers.Count} active customer(s)"
            : $"{_visibleCustomers.Count} of {_allCustomers.Count} active customer(s)";
    }
