...
public sealed class UserRepository : Repository<User>, IUserRepository
{
    ...
    public async Task<IEnumerable<User>> SearchAsync(string keyword)
    {
        ...
        if (string.IsNullOrWhiteSpace(normalized)) return await GetAllAsync();
        return await _dbSet.AsNoTracking()
            .Where(user => user.Name.Contains(normalized) || user.Email.Contains(normalized) || user.Phone.Contains(normalized) || user.Code.ToString().Contains(normalized))
    ...
    public override async Task DeleteAsync(int id)
    {
        var isReferenced =
            ...
            await _context.Contracts.AnyAsync(c => c.UserId == id);
